...
use imageproc::{
    geometric_transformations::{rotate, Interpolation::Bicubic},
    rect::Rect,
};
use ndarray::{arr2, s, Array, Array1, Array2, Array3, Array4, ArrayBase, Axis, Dim, OwnedRepr};
use std::cmp::{max, min};
...
pub fn postprocess_palms(imshape: (i32, i32), boxes: Array2<f32>, threshold: f32) -> Array2<f32> {
    ...
    for pred in boxes.axis_iter(Axis(1)) {
        let (pd_score, box_x, box_y, box_size, kp0_x, kp0_y, kp2_x, kp2_y) = (
        ...
        if box_size > 0. {
            ...
            sqn_rr_center_y =
                (sqn_rr_center_y * SQUARE_STANDARD_SIZE - SQUARE_PADDING_HALF_SIZE) / image_height;
            hands_vec.push((sqn_rr_size, rotation, sqn_rr_center_x, sqn_rr_center_y));
        }
    }
    let hands_vec_flat: Vec<f32> = hands_vec
        .into_iter()
        .flat_map(|(a, b, c, d)| vec![a, b, c, d])
        .collect(); // Flatten the Vec<(T, T)> into a Vec<T>
...
pub fn calculate_rects(palm_dets: Array2<f32>, imshape: (i32, i32)) -> Array2<f32> {
    ...
    let rects_vec_flat: Vec<f32> = rects_vec
        .into_iter()
        .flat_map(|(a, b, c, d, e)| vec![a, b, c, d, e])
        .collect(); // Flatten the Vec<(T, T)> into a Vec<T>
...
pub fn rotate_and_crop_rectangle(
    // ChatGPT
    image: &DynamicImage,
    ...
    for rect in rects.axis_iter(Axis(1)) {
        ...
        .unwrap();
        let rotated_buffer: image::ImageBuffer<_, Vec<_>> = rotate(
            &image_buffer,
            (cx, cy),
            -a.to_radians(),
            Bicubic,
        ...
        let crop = rotated.crop_imm(
            x1.min(rotated.width() - 1),
...
pub fn rotate_points_around_z_axis(
    points: Array2<f32>,
    center: [f32; 2],
    angle_degrees: f32,
) -> Array2<f32> {
...
pub fn rotate_points_around_z_axis_and_scale(
    points: Array2<f32>,
    center: [f32; 2],
    scale: f32,
    angle_degrees: f32,
